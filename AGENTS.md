...
**Utility Functions:**
- `formatBudget(number)` - Format currency with ₦
...
```
Admin creates property → API saves to database
                      ↓
Property loaded on dashboard → Display with stats
                      ↓
User views properties → Public page fetches from API
                      ↓
User books viewing → Creates booking record
                      ↓
Admin confirms booking → Email sent to user
                      ↓
Viewing completed → Booking status updated
```

### **Project Lifecycle**

```
Admin creates project → Stored in database
                     ↓
Project added to dashboard → Display with progress
                     ↓
Admin updates completion → Percentage updated
                     ↓
Public views projects → Featured projects displayed
                     ↓
Project completed → Status changed to "Completed"
```

### **User Authentication Flow**

```
User enters credentials → Login endpoint validates
                       ↓
Password hashed and compared → Tokens generated
                              ↓
Access token (15m) + Refresh token (7d) returned
                       ↓
Token stored in HTTP-only cookie
                       ↓
Subsequent requests → Token included in Authorization header
                       ↓
Middleware verifies token → Request proceeds or rejected
                       ↓
...
## 🚨 CRITICAL ISSUES & NEEDS IMPROVEMENT
...
#### **1.1 🔴 NO DATABASE CONFIGURED - CRITICAL**
...
**Editable Files:**
- ✅ `backend/server.js` - Can refactor database initialization
- ✅ `.env` / `.env.example` - Add proper database config template
- ✅ `backend/package.json` - Can add migration tools
...
#### **1.2 🔴 EMAIL HARDCODED TO GMAIL - CRITICAL**
**Status:** ❌ NOT PRODUCTION READY  
**File:** `backend/server.js` (lines 254-269)  
**Problem:**
```javascript
const transporter = nodemailer.createTransport({
  service: 'gmail',  // ← HARDCODED - only supports Gmail
  auth: {
...
**Editable Files:**
- ✅ `backend/server.js` - Refactor email transport to be configurable
...
#### **1.3 🔴 NO TOKEN REFRESH HANDLING - CRITICAL**
**Status:** ❌ NOT PRODUCTION READY  
...
**Example Flow (Broken):**
```
User working → 15min pass → Access token expires
→ Next API call returns 401 
→ User forcibly redirected to /admin/login 
...
**Editable Files:**
- ✅ `src/services/api.js` - Add response interceptor for 401 + refresh retry
...
#### **1.4 🔴 NO BOOKING DOUBLE-BOOKING PREVENTION - CRITICAL**
**Status:** ❌ NOT PRODUCTION READY  
...
**Current Code:**
```javascript
// ❌ NO VALIDATION - accepts any booking without checking conflicts
app.post('/api/bookings', async (req, res) => {
  const missing = requireFields(payload, ['name', 'email', 'phone', 'date', 'time']);
  if (missing.length > 0) return error;
  // ← MISSING: Check if this date/time already booked for this property
  // → Just inserts immediately
...
**Editable Files:**
- ✅ `backend/server.js` - Add conflict detection query before insert
- ✅ `src/services/api.js` - Add endpoint to check available slots
- ✅ `src/views/` - Add slot availability display on booking form
...
#### **1.5 🔴 HARDCODED CORS ALLOWED ORIGINS - SECURITY ISSUE**
**Status:** ❌ NOT PRODUCTION READY  
...
**Editable Files:**
- ✅ `backend/server.js` - Make CORS configurable
...
#### **2.1 🟠 HARDCODED DEFAULT ADMIN PASSWORD**
...
**Editable Files:**
- ✅ `backend/server.js` - Require `ADMIN_PASSWORD` to be set
...
#### **2.2 🟠 NO INPUT VALIDATION ON CRITICAL FIELDS**
...
**Example:**
```javascript
// ❌ This would be accepted by backend:
{
  title: "",                    // empty title
...
**Editable Files:**
- ✅ `backend/server.js` - Add validation helper function, use in all POST/PUT endpoints
- ✅ `src/views/admin/` - Add client-side validation
...
#### **2.3 🟠 AUTH STORE DOESN'T HANDLE ERRORS GRACEFULLY**
**Status:** ⚠️ UX ISSUE  
...
const login = async (email, password) => {
  try {
    ...
    // ← Error returned but never stored in state
    // ← Component must handle error display manually
  }
}
```
...
**Editable Files:**
- ✅ `src/stores/authStore.js` - Add error state
...
#### **2.4 🟠 PROPERTY/PROJECT STORES MISSING IMAGES**
...
**Editable Files:**
- ✅ `src/stores/propertyStore.js` - Add image management
- ✅ `src/stores/projectStore.js` - Add image management
- ✅ `src/views/admin/AdminPropertyForm.vue` - Add multi-image uploader
...
#### **3.1 🟡 NO PAGINATION ON PROPERTY/PROJECT LISTS**
...
app.get('/api/properties', async (_req, res) => {
  ...
  // ← Loads ALL properties every time, no pagination
...
**Editable Files:**
- ✅ `backend/server.js` - Add `?page=1&limit=20` query parameters
- ✅ `src/services/api.js` - Support pagination params
- ✅ `src/stores/propertyStore.js` - Implement pagination state
...
#### **3.2 🟡 NO ERROR BOUNDARIES / GLOBAL ERROR HANDLING**
**Status:** ⚠️ UX ISSUE  
...
**Editable Files:**
- ✅ `src/main.js` - Add global error handler
- ✅ `src/services/api.js` - Add error logging
- ✅ New component: `src/components/ErrorBoundary.vue`
...
#### **3.3 🟡 NO SEARCH/FILTER ON BACKEND**
**Status:** ⚠️ INCOMPLETE FEATURE  
...
**Editable Files:**
- ✅ `backend/server.js` - Add query parameter filters
...
#### **3.4 🟡 BOOKING STORE LOADS ALL BOOKINGS**
**Status:** ⚠️ PERFORMANCE ISSUE  
...
**Editable Files:**
- ✅ `src/stores/bookingStore.js` - Add optional filters
...
**Editable Files:**
- ✅ `backend/server.js` - Add audit logging on mutations
...
|-----------|--------|--------|
| **Database** | 🔴 CRITICAL | Not configured, no persistence |
| **Email** | 🔴 CRITICAL | Hardcoded to Gmail only |
| **Auth** | 🔴 CRITICAL | No token refresh, default password |
| **Bookings** | 🔴 CRITICAL | No double-booking prevention |
| **CORS** | 🔴 CRITICAL | Hardcoded localhost origins |
| **Input Validation** | 🟠 HIGH | Missing on numeric/enum fields |
| **Error Handling** | 🟠 HIGH | No global error handling |
| **Pagination** | 🟡 MEDIUM | Missing on large lists |
| **Audit Logging** | 🟡 MEDIUM | No change tracking |
| **Search** | 🟡 MEDIUM | Only frontend filtering |
| **Images** | 🟡 MEDIUM | Multiple images not supported |

**Overall Status:** 🔴 **NOT PRODUCTION READY**  
**Estimated Time to Production:** 20-25 hours (Priority 1-2 items)  
**Critical Path:** Database → Email → Auth → CORS → Input Validation

---

## 📝 Notes on File Editability

**Can Edit Freely:**
- ✅ `backend/server.js` - All endpoints, middleware, config
- ✅ `backend/package.json` - Dependencies, scripts
- ✅ `.env` / `.env.example` - Environment variables
- ✅ `src/stores/*.js` - All store files
- ✅ `src/services/api.js` - API client
- ✅ `src/views/admin/*.vue` - Admin components
...
- ⚠️ `PROJECT_SUMMARY.md` - Update as work progresses
- ⚠️ `AGENTS.md` - This file, update as issues fixed
- ⚠️ `DEPLOYMENT.md` - Add production URLs when deployed
...
**Production Status:** 🔴 BLOCKED - Waiting on Priority 1 fixes
